...
//#include <WinSock2.h>
	...
	char mg_http_folder[512] = "C:/si/public";
	...
	if (ev == MG_EV_HTTP_MSG) {
		...
		if (mg_vcmp(&hm->uri, "/save") == 0) {
			...
			{				
				...
				for (int i = 0;i < 10; i++) {
					...
					if (ports_type[i] == 1) { //OUTPUT
						char is_on_manual[20];
						mg_http_get_var(&hm->body, "is_on_manual", is_on_manual, sizeof(is_on_manual));
						if (strcmp(is_on_manual, "true") == 0) {
							mg_http_get_var(&hm->body, port_name, mg_ports[i], sizeof(mg_ports[i]));
						}
					}
				}
		...
		/*
			...
			if (mg_vcmp(&hm->method, "POST") == 0) {
				...
				string result = sendPrologWebRequest(request, prologServer, prologPort, status_result);
