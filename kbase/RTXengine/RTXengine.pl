...
backward_2(not(Goal),FiredRules,FiredRules):-
    !,
    writeln(Goal),
    test_with_catch('backward_not',not(Goal), true).


backward_2(Goal,FiredRules,FiredRules):-
    !,
    test_with_catch('backward',Goal,  false).
...
test_forward(Facts, FinalFacts):-
    retractall_rtx,
    forall( member(F, Facts), assert_rtx(F)),
    forward(FiredRules),
    print_fired_rules(FiredRules),
    findall(F, rtx(F), FinalFacts).


test_backward(Goal, Facts):-
    retractall_rtx,
    forall( member(F, Facts), assert_rtx(F)),
